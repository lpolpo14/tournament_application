...
async function loadTeams() {
  ...
  try {
    ...
    error.value = t('teams.errors.loadFailed')
  } finally {
    loading.value = false
  }
}
...
async function createTeam() {
  if (!canManageTeams.value) {
    error.value = t('teams.errors.createForbidden')
    return
  }
  ...
  try {
    ...
    successfulAdd.value = t('teams.success.created')
    ...
    error.value = err.response?.data?.detail || t('teams.errors.createFailed')
  }
}
                ...
                <div class="flex items-center gap-4">
                  <div class="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-gray-200 bg-gray-100">
                    <img
                      v-if="team.logo_url"
                      :src="team.logo_url"
                      :alt="t('teams.logoAlt', { name: team.team_name })"
